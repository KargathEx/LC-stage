...
update:deque转成vector只多过了一个例子。
```cpp

class Solution {
  set<vector<int>> no_dup; //不能记忆化，至少避免传参了。
  //改自930，那个是0/1串，在这里成了A[i]%p==0
  void numSubarraysWithSum(vector<int>& A, int S, int p) {
    int res = 0, sum = 0, left = 0, n = A.size();
    deque<int> temp;
    for (int i = 0; i < n; ++i) {
      ...
      if (sum == S) { //先把当前塞进去,然后再收缩右边界
        if (!temp.empty()) {
          vector<int> sa{ begin(temp),end(temp) };
          if (no_dup.find(sa) != no_dup.end())continue;
          no_dup.insert(sa);  //这里把输入转换为vec,再插入。
        }//防空
      }
      for (int j = left; j < i && (A[j] % p != 0); ++j) {  //判定为当前不为可以被p整除的数字，即==0的情况。
        if (temp.empty())continue;
